"""
Filename: ms02_Identify_min_control_sets_FVS.py
Author: Yoshiaki Fujita
Date: 2025-09-21
Description:
    This script is to identify the minimum control sets as FVS, with cana feedback_vertex_set_driver_nodes(), 
    from either structural graph or effective graphs of multiple models specified in an input csv.

Usage:
    python ms02_Identify_min_control_sets_FVS.py [options]
...
Example:
    python ms02_Identify_min_control_sets_FVS -n 10 -s 1 -g EG
"""
...
def identify_min_control_sets_SG(df, sModel_Path, sOut_Path, nProcess_Models, max_nodes, start_model):
    """
    ...
    sOutFileName = "/FVS_structural.csv"
    ...
    for i in range(len(df)):
        ...
        nNodes_s, nEdges_s, min_sets = bn.feedback_vertex_set_driver_nodes(max_search=max_nodes, method='bruteforce')
...
def identify_min_control_sets_EG(df, sModel_Path, sOut_Path, nProcess_Models, max_nodes, start_model):
    """
    ...
    sOutFileName = "/FVS_effective.csv"
    ...
    for i in range(len(df)):
        ...
        # Get the number of nodes and number of edges of structual graph
        SG = bn.structural_graph()
        nNodes_SG = SG.number_of_nodes()
        nEdges_SG = SG.number_of_edges()
        ...
        for k in range(len(threshold_lists)):
            ...
            nNodes_e, nEdges_e, min_sets = bn.feedback_vertex_set_driver_nodes(
                    graph='effective', threshold=threshold_lists[k], max_search=max_dvs, method='bruteforce')
...
def main():
    ...
    if args.arg_g == 'SG':
        ...
        print("")
        identify_min_control_sets_SG(df, args.arg_m, args.arg_o, args.arg_n, args.arg_y, args.arg_s)
        print("")
        print(f"Finish identification from SG of {args.arg_n} models")
    elif args.arg_g == 'EG':
        print(f"Start identification from EG of {args.arg_n} models")
        print("")
        identify_min_control_sets_EG(df, args.arg_m, args.arg_o, args.arg_n, args.arg_y, args.arg_s)
        print("")
