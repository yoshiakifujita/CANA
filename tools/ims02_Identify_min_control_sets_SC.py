"""
Filename: ms02_Identify_min_control_sets_SC.py
Author: Yoshiaki Fujita
Date: 2025-09-21
Description:
    This script is to identify the minimum control sets based on SC theory, with cana structural_controllability_driver_nodes(), 
...
def identify_min_control_sets_SG(df, sModel_Path, sOut_Path, nProcess_Models, start_model):
    """
    ...
    sOutFileName = "/SC_structural.csv"
    ...
    for i in range(len(df)):
        ...
        nNodes_e, nEdges_e, min_sets = bn.structural_controllability_driver_nodes()
...
def identify_min_control_sets_EG(df, sModel_Path, sOut_Path, nProcess_Models, start_model):

    threshold_lists = [0.0, 0.2, 0.4, 0.6, 0.8]

    sOutFileName = "/SC_effective.csv"
    ...
    for i in range(len(df)):
        ...
        for k in range(len(threshold_lists)):
            ...
            # Identify minimum control sets
            nNodes_e, nEdges_e, min_sets = bn.structural_controllability_driver_nodes(graph='effective', threshold=threshold_lists[k])
...
def main():
    ...
    if args.arg_g == 'SG':
        ...
        print("")
        identify_min_control_sets_SG(df, args.arg_m, args.arg_o, args.arg_n, args.arg_s)
        print("")
        print(f"Finish identification from SG of {args.arg_n} models")
    elif args.arg_g == 'EG':
        print(f"Start identification from EG of {args.arg_n} models")
        print("")
        identify_min_control_sets_EG(df, args.arg_m, args.arg_o, args.arg_n, args.arg_s)
        print("")
